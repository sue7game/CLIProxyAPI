...
// 2. Use auth.EffectiveProxyURL if configured
...
func NewProxyAwareHTTPClient(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, timeout time.Duration) *http.Client {
	...
	// Priority: request override, then effective auth proxy, then cfg.ProxyURL.
	...
	// Use RoundTripper from context when no configured proxy is available.
